...
class _PDFCompressorScreenState extends State<PDFCompressorScreen> {
  ...
  Future<void> _compressPDF() async {
    if (_selectedFile == null) {
      _showSnackBar('Please select a PDF file to compress.', isError: true);
      return;
    }
    ...
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse(ApiConfig.pdfCompressEndpoint),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final file = File('${dir.path}/compressed_${_selectedFile!.name}');
        ...
        setState(() => _isLoading = false);
        _showSnackBar(
          'Failed to compress PDF. Status: ${response.statusCode}',
          isError: true,
        );
      }
    } catch (e) {
      setState(() => _isLoading = false);
      _showSnackBar('Error compressing PDF: $e', isError: true);
    }
  }
  ...
  void _showSuccessDialog(File file) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Your PDF has been compressed. Result size: ${(file.lengthSync() / 1024).toStringAsFixed(1)} KB',
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Share.shareXFiles([
                XFile(file.path),
              ], text: 'Here is my compressed PDF!');
            },
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      'Reduce PDF file size without significant visual quality loss.',
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Original: ${(_selectedFile!.size / 1024).toStringAsFixed(1)} KB',
                                  style: GoogleFonts.outfit(
                                    color: DesignSystem.secondaryText(context),
                                  ),
                                ),
