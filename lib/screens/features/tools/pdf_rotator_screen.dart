...
class PDFRotatorScreen extends StatefulWidget {
  const PDFRotatorScreen({super.key});

  @override
  State<PDFRotatorScreen> createState() => _PDFRotatorScreenState();
}

class _PDFRotatorScreenState extends State<PDFRotatorScreen> {
  ...
  Future<void> _rotatePDF() async {
    ...
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse(ApiConfig.pdfRotateEndpoint),
      );
      ...
      request.fields['rotate'] = _rotation.toString();
      ...
      if (response.statusCode == 200) {
        ...
        final file = File('${dir.path}/rotated_${_selectedFile!.name}');
        ...
        setState(() => _isLoading = false);
        _showSnackBar(
          'Failed to rotate PDF. Status: ${response.statusCode}',
          isError: true,
        );
      }
    } catch (e) {
      setState(() => _isLoading = false);
      _showSnackBar('Error rotating PDF: $e', isError: true);
    }
  }
  ...
  void _showSuccessDialog(File file) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Your PDF has been rotated by $_rotation degrees.',
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      'Correct the orientation of pages in your PDF.',
              ...
              children: [90, 180, 270]
                  .map(
                    (angle) => ChoiceChip(
                      ...
                      onSelected: (selected) {
                        if (selected) setState(() => _rotation = angle);
                      },
                      selectedColor: DesignSystem.primaryPurple,
                      labelStyle: TextStyle(
                        color: _rotation == angle
                            ? Colors.white
